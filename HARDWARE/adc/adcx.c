...
{
	//配置ADC通道
	ADC_RegularChannelConfig(ADCx, ADC_Channel, 1, ADC_SampleTime);//在规则组菜单列表的第一个位置，写入某个通道。
	//虽然存在覆盖问题，同一个序列填了不同的通道，但只要保证两次采集调用是独立的（先后依次执行），实际也能获取到正确结果，
	//例子ADC_RegularChannelConfig(ADCx, ADC_Channe2, 2, ADC_SampleTime);在序列2上配置通道2
